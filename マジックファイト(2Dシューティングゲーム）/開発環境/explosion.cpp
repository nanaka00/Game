...
LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffExplosion = NULL;				// 頂点バッファへのポインター
...
HRESULT InitExplosion(void)
{
	...
	for (int nCntExplosion = 0; nCntExplosion < MAX_EXPLOSION; nCntExplosion++, pVtx += 4)
	{
		...
		pVtx[2].tex = D3DXVECTOR2(0.125f, 0.5f);
...
void SetVertexExplosion(int nPatternAnim,int nCntExplosion)
{
	...
	// 頂点情報の設定(Nの順番）
	pVtx[0].pos = g_aExplosion[nCntExplosion].pos + D3DXVECTOR3(-(g_aExplosion[nCntExplosion].size.x ), +(g_aExplosion[nCntExplosion].size.y ), 0.0f);
	pVtx[1].pos = g_aExplosion[nCntExplosion].pos + D3DXVECTOR3(-(g_aExplosion[nCntExplosion].size.x ), -(g_aExplosion[nCntExplosion].size.y ), 0.0f);
	pVtx[2].pos = g_aExplosion[nCntExplosion].pos + D3DXVECTOR3(+(g_aExplosion[nCntExplosion].size.x ), +(g_aExplosion[nCntExplosion].size.y ), 0.0f);
	pVtx[3].pos = g_aExplosion[nCntExplosion].pos + D3DXVECTOR3(+(g_aExplosion[nCntExplosion].size.x ), -(g_aExplosion[nCntExplosion].size.y ), 0.0f);

	pVtx[0].tex = D3DXVECTOR2(nPatternAnim * 0.125f + 0.0f, 0.5f);
	pVtx[1].tex = D3DXVECTOR2(nPatternAnim * 0.125f + 0.0f, 0.0f);
	pVtx[2].tex = D3DXVECTOR2(nPatternAnim * 0.125f + 0.125f, 0.5f);
